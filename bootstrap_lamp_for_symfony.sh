...
echo "Installing GIT and ACL (POSIX Access Control Lists)"
apt-get install git acl -y > /dev/null
...
apt-get install -y php5 php5-cli php5-fpm curl php5-curl php5-mcrypt php5-xdebug php5-gd php5-apcu php5-json php5-readline
...
echo "Creating a symbolic link for nodejs"
...
echo "Activating ACL for Main Partition"
sed -i '/default/c default,acl' /etc/fstab
